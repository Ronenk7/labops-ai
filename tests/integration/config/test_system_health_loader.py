"""Integration tests for external system health configuration loading."""
import json
from pathlib import Path

import pytest

from labops_ai.config import SystemHealthConfig, SystemHealthConfigLoader
from tests.support.fixture_loader import load_test_fixture


CASES = load_test_fixture("config/system_health_loader_cases.json")


@pytest.mark.integration
class TestSystemHealthConfigLoader:
    """Verify cooperation between JSON files and system health models."""
    ...
    def test_loads_default_project_configuration(self) -> None:
        ...
        assert isinstance(config, SystemHealthConfig)
        assert config.collection.cpu_sample_interval_seconds > 0.0
        assert config.collection.disk_mount_point
        assert set(config.metric_thresholds) == set(
            config.report.metric_labels
        )

    def test_loads_custom_configuration(self, tmp_path: Path) -> None:
        """Verify conversion of custom JSON into configuration models."""
        config_path = tmp_path / "system_health.json"
        config_path.write_text(
            json.dumps(CASES["valid_custom_configuration"]),
            encoding="utf-8",
        )

        config = SystemHealthConfigLoader(config_path).load()
        expected = CASES["valid_custom_configuration"]

        assert config.collection.cpu_sample_interval_seconds == expected[
            "collection"
        ]["cpu_sample_interval_seconds"]
        assert config.collection.disk_mount_point == expected[
            "collection"
        ]["disk_mount_point"]
        assert config.report.title == expected["report"]["title"]
    ...
    def test_rejects_missing_required_section(
        ...
        with pytest.raises(ValueError, match="Missing required keys"):
            SystemHealthConfigLoader(config_path).load()
    ...
    def test_rejects_missing_configuration_file(
        ...
        with pytest.raises(FileNotFoundError, match="was not found"):
            SystemHealthConfigLoader(missing_path).load()